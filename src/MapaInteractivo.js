...
const defaults = {
  ...
  baseLayer: {
    ...
    params: {
      ...
      minZoom: 9,
      attribution:
        '<a href="https://usig.buenosaires.gob.ar" target="_blank">USIG</a> (<a href="http://www.buenosaires.gob.ar" target="_blank">GCBA</a>), © <a href="http://www.openstreetmap.org/copyright/en" target="_blank">OpenStreetMap</a> (ODbL)',
      tms: true,
      id: "",
      accessToken: "",
    },
  ...
  layers: {
    apiUrl: "https://geoserver-dev.gcba.gob.ar/geoserver",
...
class MapaInteractivo {
  constructor(nodeId, options) {
    ...
    this._markersClusterLayerGroup = new MarkerClusterGroup({
    ...
    this.baseLayer = L.tileLayer(
      this.config.baseLayer.uri,
      this.config.baseLayer.params
    );
    ...
    for (let id in this._recorridos) {
      ...
      if (id === r.id) {
        if (!auxRecorrido._layer) {
          // this.ocultarRecorrido();
          this._recorridos[r.id] = Object.assign({}, r, {
            estilo: r.setOpacity
              ? { opacity: 0.65, markerOpacity: 1 }
              : { opacity: 0.35, markerOpacity: 0.35 },
          });
          this._recorridos[r.id]._layer = recorrido(
            this._recorridos[r.id],
            r.tipo === "car" || r.tipo === "walk" ? null : Coords.toLngLat
          );
          this._recorridos[r.id]._layer.on("click", (e) =>
            this.recorridoClick(e, r.id)
          );
          ...
          auxRecorrido.estilo = r.setOpacity
            ? { opacity: 0.65, markerOpacity: 1 }
            : { opacity: 0.35, markerOpacity: 0.35 };
          this._layerGroup.removeLayer(auxRecorrido._layer);
          this._recorridos[auxRecorrido.id]._layer = recorrido(
            auxRecorrido,
            auxRecorrido.tipo === "car" || auxRecorrido.tipo === "walk"
              ? null
              : Coords.toLngLat
          );
          this._recorridos[auxRecorrido.id]._layer.on("click", (e) =>
            this.recorridoClick(e, auxRecorrido.id)
          );
  ...
  mostrarPaso(recorridoId, pasoId) {
    ...
    if (pasoId === "end") {
      ...
      this._recorridos[recorridoId]._layer.eachLayer((geometry) => {
        if (geometry.feature.properties.paso_id === pasoId)
  ...
  mostrarMarkersRecorrido(r) {
    if (!this._mostrandoMarkersRecorrido) {
      ...
      if (r.coordOrigen) {
        coordsOrigen = Coords.toLngLat(
          r.coordOrigen.split(",").map(parseFloat)
        );
        coordsDestino = Coords.toLngLat(
          r.coordDestino.split(",").map(parseFloat)
        );
      } else {
      ...
      this.toMarkerId = this.addMarker(
        coordsDestino,
        true,
        false,
        false,
        false,
        false,
        { icon: this.config.toMarker }
      );
      this.fromMarkerId = this.addMarker(
        coordsOrigen,
        true,
        false,
        false,
  ...
  _loadMapsDefs() {
    if (!this._loadingMaps && !this.mapsDefs) {
      ...
      let mapsPromise = fetch(
        this.config.layers.apiUrl + "mapainteractivoba/mapas/?protocol=https"
      )
        .then((res) => res.json())
        .then((mapsDefs) => {
  ...
  _loadLayer(layerName, layerGroup) {
    ...
    Object.entries(conf).forEach((layer) => {
      switch (layer[1].format) {
        case "geojson":
          this.msgControl.show(
            this.config.texts[this.config.language].loadingInformation
          );
          ...
          fetch(layer[1].url)
            ...
            .catch((err) => {
              if (self._layers[layerName]) {
                console.error(err);
                self.msgControl.show(
                  this.config.texts[this.config.language]
                    .errorLoadingInformation
                );
                this._loadingLayer = false;
              }
  ...
  _onFeatureClick(e) {
    ...
    const layerId =
      e.target && e.target.options ? e.target.options.layerId : "";
    const layerName =
  ...
  _addLayer(layerName, layerId, layerGroup, data) {
    ...
    try {
      if (this._layerBuilders[builder]) {
        ...
        this._layers[layerName][layerId].on(
          "click",
          this._onFeatureClick.bind(this)
        );
  ...
  addPublicLayer(layerName, options = {}) {
    if (!this._layers[layerName]) {
      ...
      if (this.layersDefs) {
        ...
        if (layer) {
          if (!this._layers[layerName]) {
            this._layers[layerName] = Object.assign(
              {},
              this._layers[layerName],
              options
            );
        ...
        const self = this;
        this.msgControl.show(
          this.config.texts[this.config.language].loadingLayers
        );
    ...
    if (this.layersDefs) {
      const layer =
        layerName.indexOf(".") === -1
          ...
          : {
              [layerName]:
                this.layersDefs[layerName.split(".")[0]][
                  layerName.split(".")[1]
                ],
            };
  ...
  setBaseLayer(baseLayer) {
    ...
    if (!baseLayer) {
      ...
      this.baseLayer = L.tileLayer(
        baseLayer.uri,
        Object.assign({}, baseLayer.params, this.config.baseLayer.params)
      );
    } else {
  ...
  _onClick(ev) {
    ...
    _.forOwn(this._layers, (layer, layerName) => {
      if (layer.callAPIOnClick) {
        const url = (
          layer.onClickAPI ||
          this.config.layers.apiUrl + this.config.layers.reverseGeocodeUrl
        )
  ...
  selectMarker(markerId) {
    this.inactivateMarker();
    const marker =
      markerId === "-1" ? this._locationMarker : this._markers[markerId];
  ...
  isMarkerActive(markerId) {
    return (
      this._activeMarker && this._activeMarker.options.markerId === markerId
    );
  }
  _onMarkerDragEnd(ev) {
    // console.log(ev);
    if (
      this._activeMarker &&
      ev.target.options.markerId !== this._activeMarker.options.markerId
    ) {
      this.inactivateMarker();
  ...
  addMarker(
    ...
    const existentMarker = _.map(this._markers, (value) => value).filter(
      (marker) => marker.options.id === (options.id || id)
    )[0];
  ...
  getLocationClass(pos) {
    return Date.now() - pos.timestamp > 10000 || pos.coords.accuracy > 200
      ? "gray"
      : "";
  }
  ...
  addLocationMarker(pos, recenter, zoomIn) {
    ...
    if (!this._locationMarker) {
      ...
      this._locationMarker.setLatLng(
        L.latLng(pos.coords.latitude, pos.coords.longitude)
      );
    } else {
      ...
      if (options && options.url) {
        ...
        if (layer) {
          ...
          if (options.displayPopup) {
            const eventListener = options.displayPopup.onEvent
              ? options.displayPopup.onEvent
              : "click";
        ...
        console.log(
          "Error: es necesario pasar la url del tile service entre las opciones"
        );
      }
    }
  }
